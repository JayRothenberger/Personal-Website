...
# globals
try:
    riot_api_key = APIKey.objects.filter(service__exact='Riot').order_by('-time')[:1][
        0].key  # this should come from db in production
...
def APICall(call='matchlist_by_account', args={'summoner': 'MomDom', 'region': 'na1'}):
    try:
    ...
    if calls_ago_100 > two_minutes_ago or calls_ago_20 > one_second_ago:
        ...
        try:
            ...
            if call == 'matchlist_by_account':
                return watcher.match.matchlist_by_account(args['region'], args['accountId'],
...
def updateMatchHistory(summoner='MommyDommy', region='na1', return_latest=True, to_return=1):
    ...
    print(me)
    my_matches = APICall(call='matchlist_by_account',
                         args={'region': region, 'accountId': me['accountId'], 'begin_time': begin,
    ...
    while len(my_matches) > 0:
        try:
            ...
            my_matches.pop(0)
            to_store = MatchHistory(time=int(match_detail['gameCreation']), data=json.dumps(match_detail),
                                    summoner=summoner, region=region)
            to_store.save()
        except Exception as e:
            print(e)

    match_detail = MatchHistory.objects.filter(summoner__iexact=summoner, region__iexact=region).order_by('-time')[
                   :to_return]
    rax = []
...
def updateProducts(loc_id_list=getLocIds()):
    ...
    for i in loc_id_list:
        ...
        for brand in brands:
            start = 0
            URL4 = 'https://api.kroger.com/v1/products?filter.term=beer&filter.brand=' + brand.replace(" ",
                                                                                                       "%20").replace(
                "'", "%27").replace("&", "%26") + '&filter.locationId=' + i + \
                   '&filter.limit=50'
            ...
            while g['data'] != []:
                ...
                URL4 = 'https://api.kroger.com/v1/products?filter.term=beer&filter.brand=' + brand.replace(" ",
                                                                                                           "%20").replace(
                    "'", "%27").replace("&", "%26") + '&filter.locationId=' \
        ...
        for key in price_size.keys():
            try:
                z = (price_size[key][0] * abv_dict[products[key]] * .01, price_size[key][1], price_size[key][2],
                     price_size[key][3])
                data.append(z)
            except Exception as e:
                print(e, ': error transforming price_size data')
        # now price_size contains the dollars per ml of alcohol
        data.sort(reverse=True, key=lambda k: k[0])
        if data[:5] != []:
            to_store = KrogerServiceData(int(time.time() * 1000), json.dumps(
                {'brands': brands, 'abv_dict': abv_dict, 'products': products, 'price_size': price_size,
                 'loc_id_list': loc_id_list, 'cheapest5': data[:5]}))
            to_store.save()
...
def main(q_arr, min_votes=25):
    ...
    for x, y in queries_bias:
        if max(x) < 0:
            bias_warning = "results for " + y + " seem to be left-leaning.  Consider reformulating your query."
            if mutate_query(y):
                suggestions = mutate_query(y)
                print("you could try:", suggestions)
        if min(x) > 0:
            bias_warning = "results for " + y + " seem to be right-leaning.  Consider reformulating your query."
            if mutate_query(y):
                suggestions = mutate_query(y)
                print("you could try:", suggestions)
        bias_tuple = x
...
def upload_file(request):
    if request.method == 'POST':
        ...
        # if form.is_valid():
        ret = 'image uploaded as ' + handle_image_upload(request.FILES['image'], request.FILES['image'].name,
                                                         request.POST['iid'], request.POST['tags'])
...
def create_user(request):
    if request.method == 'POST':
        ...
        # if form.is_valid():
        ret = 'user created as ' + handle_create_new_user(request.FILES['resume'], request.POST['pid'],
                                                          request.POST['pword'], request.POST['bio'],
                                                          request.POST['dname'])
        return render(request, 'personal/success.html', {'actionID': ret})
    else:
...
def riotDashboard(request):
    ...
    for match_detail in match_details:
        ...
        score_headline = {'team100': {'kills': sum(df['kills'][:5]), 'deaths': sum(df['deaths'][:5]),
                                      'assists': sum(df['assists'][:5])},
                          'team200': {'kills': sum(df['kills'][5:]), 'deaths': sum(df['deaths'][5:]),
                                      'assists': sum(df['assists'][5:])}}

        team100KDA = str(score_headline['team100']['kills']) + '/' + str(
            score_headline['team100']['deaths']) + '/' + str(score_headline['team100']['assists'])
        team200KDA = str(score_headline['team200']['kills']) + '/' + str(
            score_headline['team200']['deaths']) + '/' + str(score_headline['team200']['assists'])
        ...
        for index, row in df.iterrows():
            if index == 0:
                data['content'].append({'text': [textd(match_detail['teams'][0]['win'].replace('Fail', 'Loss')),
                                                 textd(team100KDA), textd('Gold Earned: ' + team100Gold)],
                                        'images': []})
            if index == 5:
                data['content'].append({'text': [textd(match_detail['teams'][1]['win'].replace('Fail', 'Loss')),
                                                 textd(team200KDA), textd('Gold Earned: ' + team200Gold)],
                                        'images': []})
            ...
            stats = {'text': [], 'images': [],
                     'bgcolor': 'gray'}  # list of textds and imageds to be displayed on page and their bg color

            stats['images'].append(imaged('http://ddragon.leagueoflegends.com/cdn/img/' + line['keystone_path']))
            stats['images'].append(imaged(
                url='http://ddragon.leagueoflegends.com/cdn/' + latest + '/img/champion/' + line[
                    'championName'] + '.png', width=50, height=50, title=line['championName']))
            stats['images'].append(imaged(url='https://i.imgur.com/Z4PgTUN.png'))
            try:  # bots do not take summoner spells
                stats['images'].append(imaged(
                    url='http://ddragon.leagueoflegends.com/cdn/' + latest + '/img/spell/' + spell_dict[
                        str(int(line['spell1']))] + '.png'))
                stats['images'].append(imaged(
                    url='http://ddragon.leagueoflegends.com/cdn/' + latest + '/img/spell/' + spell_dict[
                        str(int(line['spell2']))] + '.png'))
            except:
                pass
            stats['images'].append(imaged(url='https://i.imgur.com/Z4PgTUN.png'))
            stats['images'].append(imaged(url='http://ddragon.leagueoflegends.com/cdn/' + latest + '/img/item/' + str(
                int(line['item0'])) + '.png'))
            stats['images'].append(imaged(url='http://ddragon.leagueoflegends.com/cdn/' + latest + '/img/item/' + str(
                int(line['item1'])) + '.png'))
            stats['images'].append(imaged(url='http://ddragon.leagueoflegends.com/cdn/' + latest + '/img/item/' + str(
            ...
            if line['win'] == True:
                ...
                if line['summonerName'] == summoner_clean:
                    losses += 1
            stats['text'].append(
                textd(value=str(line['kills']) + '/' + str(line['deaths']) + '/' + str(line['assists'])))
